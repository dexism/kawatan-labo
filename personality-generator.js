...
const personalityDefinitions = {
    ...
    "modifiers": { 
        "嫌悪": "排他的な", "独占": "独占欲の強い", "依存": "かまってほしがりな", "執着": "執念深い", "恋心": "ロマンチストな", "対抗": "負けず嫌いの", "友情": "友好的な", "保護": "過保護な", "憧憬": "理想主義の", "信頼": "献身的な" 
    },
    "special_cores": { 
...
const relationshipDefinitions = {
    ...
    "保護,友情": "友のために、危険を顧みずに行動する献身的な守護者",
    "保護,執着": "「守るため」という大義名分で、相手を常に監視する看守",
    "保護,嫌悪": "守りたいはずが、その弱さを憎んでしまう歪んだ庇護",
    "保護,対抗": "見下している相手を守らざるを得ない、屈辱的な庇護関係",
    "保護,恋心": "庇護欲を恋愛感情と錯覚している守護者と、それに甘える姫",
    "保護,憧憬": "神聖な憧れの対象を、どんな脅威からも守ろうとする狂信的な騎士",
    "保護,独占": "保護の名目で相手を囲い込み、外部から隔離する支配者",

    "信頼,友情": "固い友情と信頼で結ばれた、戦場における唯一の心の支え",
    "信頼,執着": "信頼を逆手に取り、相手のプライベートに踏み入る侵害者",
    "信頼,嫌悪": "信じようとする度に裏切られたと感じる、人間不信の対象",
    "信頼,対抗": "信頼する相手だからこそ、絶対に負けたくない真剣な勝負相手",
    "信頼,恋心": "信頼から芽生えた、盲目的で純粋な恋愛感情",
    "信頼,憧憬": "尊敬と信頼が入り混じった、師弟あるいはそれ以上の関係",
    "信頼,独占": "信頼関係を悪用し、相手の交友関係を支配する操縦者",

    "友情,執着": "親友という立場を利用し、どこまでもついてくる影",
    "友情,嫌悪": "友情を求めながらも、素直になれず攻撃してしまう天邪鬼",
    "友情,対抗": "互いを高め合う、健全に見えて熾烈な競争を繰り広げる親友",
    "友情,恋心": "親友という関係を壊したくない、臆病な片思い",
    "友情,憧憬": "尊敬する友のようになりたいと願う、純粋な追随者",
    "友情,独占": "親友を誰にも渡さない、友情を名目にした束縛",

    "執着,嫌悪": "憎悪の対象から片時も目を離さない、粘着質な監視者",
    ...
    "対抗,恋心": "同じ相手を想う恋敵、あるいは好敵手への歪んだ恋慕",
    "対抗,憧憬": "憧れの背中をいつか超えることを誓う、好敵手兼崇拝者",
    "対抗,独占": "好敵手を自分だけのものにしたい、歪んだ独占欲を持つライバル",

    "恋心,憧憬": "手の届かない偶像に、報われないと知りながら恋い焦がれるファン",
    "恋心,独占": "愛するが故に、相手を世界から隔離したいと願う籠の管理者",

    "憧憬,独占": "憧れの対象を穢させまいと、独り占めしようとする狂信的な崇拝者"
};
